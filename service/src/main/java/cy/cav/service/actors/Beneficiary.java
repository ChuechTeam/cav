...
public class Beneficiary extends Actor {
    ...
    void requestAllowance(Envelope<RequestAllowanceNotification> envelope) {
        ...
        log.info("Processing allowance request for beneficiary: {}, type: {}", 
            notification.beneficiaryId(), notification.allowanceType());
        
        // Get beneficiary from internal state
        cy.cav.service.domain.Beneficiary beneficiary = beneficiaries.get(notification.beneficiaryId());
        if (beneficiary == null) {
            log.warn("Beneficiary not found in internal state: {}", notification.beneficiaryId());
            // Try to load from store if not in internal state (e.g., after restart)
            beneficiary = store.findBeneficiaryById(notification.beneficiaryId()).orElse(null);
            if (beneficiary != null) {
                beneficiaries.put(beneficiary.getId(), beneficiary);
                log.debug("Beneficiary loaded from store: {}", notification.beneficiaryId());
            } else {
                log.warn("Beneficiary not found in store: {}", notification.beneficiaryId());
                return;
            }
        }
        
        // TODO: La demande est créée ici de manière asynchrone, le client ne connaît pas le requestId
        // Create allowance request
        cy.cav.service.domain.AllowanceRequest request = new cy.cav.service.domain.AllowanceRequest(
            notification.beneficiaryId(), notification.allowanceType());
        ...
        world.query(null, calculatorAddress, calcRequest)
            .thenAccept(response -> {
                ...
                request.setStatus(response.accepted() ? "ACCEPTED" : "REJECTED");
                request.setAllowanceId(response.allocationId());
                request.setMonthlyAmount(response.monthlyAmount());
                request.setRejectionReason(response.rejectionReason());
                store.saveRequest(request);
                
                log.info("Allowance request {} processed: status={}, amount={}", 
                    request.getId(), request.getStatus(), request.getMonthlyAmount());
            })
            .exceptionally(throwable -> {
                log.error("Error processing allowance request", throwable);
                request.setStatus("REJECTED");
                request.setRejectionReason("Error processing request: " + throwable.getMessage());
                store.saveRequest(request);
                return null;
            });
    }
